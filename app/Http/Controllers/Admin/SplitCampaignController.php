...
class SplitCampaignController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $db_record = $db_record->where('is_split_testing', 1);
            ...
            if ($request->has('search') && $request->search != "") {
                ...
                // $db_record = $db_record->whereIn('user_id', $users);
            }
            ...
            $datatable = $datatable->addColumn('action', function ($row) {
                ...
                if (have_right(98) && ($row->status == 4 || $row->status == 5)) {
                    $actions .= '&nbsp;<a class="btn btn-primary" target="_blank" href="' . url("admin/split-campaigns/" . Hashids::encode($row->id) . '/view') . '" title="View Report"><i class="fa fa-list-alt"></i></a>';
                }
        ...
        $data = [];
        return view('admin.split-campaign.index', $data);
    }
    ...
    public function history(Request $request, $id, $hid)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('module') && $request->module != "") {
                if ($request->module == 1) { // sent to
                    ...
                    $db_record = $db_record->pluck('contact_id')->toArray();
                } elseif ($request->module == 22) { // success
                    $db_record = new EmailCampaignLogs();
                    ...
                    if ($campaign->split_test_param == 1) {
                        ...
                        $sub1 = SplitTestSubject::where('id', $campaign->split_subject_line_1)->first();
                        $db_record = new EmailCampaignLogs();
                        $db_record = $db_record->where('history_id', $history)->where('subject_id', $campaign->split_subject_line_1);
                        $db_record = $db_record->pluck('contact_id')->toArray();
                    } else {
                        $emailTemplate1 = EmailCampaignTemplate::where('id', $campaign->split_email_content_1)->first();
                        $db_record = new EmailCampaignLogs();
                        $db_record = $db_record->where('history_id', $history)->where('content_id', $campaign->split_email_content_1);
                        $db_record = $db_record->pluck('contact_id')->toArray();
                    }
                } elseif ($request->module == 10) {
                    // Split Campaign
                    $campaign = EmailCampaign::where('id', $id)->first();
                    if ($campaign->split_test_param == 1) {
                        // $campaign->split_subject_line_1;
                        $sub2 = SplitTestSubject::where('id', $campaign->split_subject_line_2)->first();
                        $db_record = new EmailCampaignLogs();
                        $db_record = $db_record->where('history_id', $history)->where('subject_id', $campaign->split_subject_line_2);
                        $db_record = $db_record->pluck('contact_id')->toArray();
                    } else {
                        $emailTemplate2 = EmailCampaignTemplate::where('id', $campaign->split_email_content_2)->first();
                        $db_record = new EmailCampaignLogs();
                        $db_record = $db_record->where('history_id', $history)->where('content_id', $campaign->split_email_content_2);
                        $db_record = $db_record->pluck('contact_id')->toArray();
                    }
                }
            }
        ...
        $data['opensData'] = $vals;
        if ($campaign && $campaign->split_test_param == 1) {
            $sub1 = SplitTestSubject::where('id', $campaign->split_subject_line_1)->first();
            $sub2 = SplitTestSubject::where('id', $campaign->split_subject_line_2)->first();
            $data['content1'] = $sub1->split_subject;
            $data['content2'] = $sub2->split_subject;
        } else {
            $emailTemplate1 = EmailCampaignTemplate::where('id', $campaign->split_email_content_1)->first();
            $emailTemplate2 = EmailCampaignTemplate::where('id', $campaign->split_email_content_2)->first();
            $data['content1'] = $emailTemplate1->image;
            $data['content2'] = $emailTemplate2->image;
        }
        return view('admin.split-campaign.history', $data);
    }
        ...
        if ($EmailCampaign && $report) {
            ...
            if ($request->module == 9) {
                ...
                if ($campaign->split_test_param == 1) {
                    ...
                    $db_record = $db_record->where('history_id', $history)->where('subject_id', $campaign->split_subject_line_1);
                    $db_record = $db_record->pluck('contact_id')->toArray();
                    $contacts = Contact::whereIn('id', $db_record)->get();
                    return Excel::download(new ReportContactsExport($contacts, $request), 'part_one.xlsx');
                } else {
                    $db_record = new EmailCampaignLogs();
                    $db_record = $db_record->where('history_id', $history)->where('content_id', $campaign->split_email_content_1);
                    $db_record = $db_record->pluck('contact_id')->toArray();
                    $contacts = Contact::whereIn('id', $db_record)->get();
                    return Excel::download(new ReportContactsExport($contacts, $request), 'part_two.xlsx');
                }
            }
    ...
    public function report($id)
    {
        ...
        return view('admin.split-campaign.report')->with($data);
    }
}
