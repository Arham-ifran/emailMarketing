...
class EmailCampaignJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $campaign = EmailCampaign::where(['user_id' => $user->id, 'id' => $campaign_id, 'job_code' => $this->job_code])->first();
        ...
        if (isset($campaign->id)) {
            ...
            if (isset($campaign->campaign_type)) {
                ...
                // $group = Group::where('id', $campaign->group_id)->with('contacts')->first();
                $contacts = [];
                ...
                if ($campaign->group_ids && count(json_decode($campaign->group_ids)) != 0) {
                    $groups = Group::whereIn('id', array_column(json_decode($campaign->group_ids), 'id'))->get();
                ...
                if (count($contacts) > 0) {
                    ...
                    Notification::create([
                        ...
                        'redirect_to' => "/email-campaign/" . \Hashids::encode($campaign->id)  . '/report',
                    ]);
                    User_log::create([
                        'user_id' => $campaign->user_id,
                        'item_id' => $campaign->id,
                    ...
                    foreach ($contacts as $contact) {
                        ...
                        if ($contact->for_email == 1 && $contact->subscribed) {
                            ...
                            if ($excludes == NULL || (is_array($excludes) && in_array($contact->id, $excludes) == false)) {
                                ...
                                try {
                                    SendCapmaignMail::dispatch([$campaign->id, $contact->id, $campaignHistory->id], [$request["sender_name"], $request["sender_email"], $request["reply_to_email"]], $contact->email, $campaign->subject, $linkDom->saveHTML() . "<img  src=" . url('track-campaign') . "?_id=" . Hashids::encode($campaign_id) . "&referal_id=" . Hashids::encode($contact->id) . "&history_id=" . Hashids::encode($campaignHistory->id) . " style='width:1px;height:1px; display: none;'>");
                                    $log = EmailSendingLog::create([
                                        'user_id' => $user->id,
                ...
                $replace = array($name, TranslationHandler::getTranslation($user->language, 'Email Campaigns'), $campaign->name, $created_at, $updated_at, $sent_at,  settingValue('site_title'));
            ...
            if ($campaign->campaign_type == 3) {
                ...
                if ($campaign->no_of_time > 0) {
                    if ($request['recursive_campaign_type'] == 1) {
                        ...
                        EmailCampaignJob::dispatch($user, $request, $campaign->id, $this->job_code)->delay($totalDuration);
