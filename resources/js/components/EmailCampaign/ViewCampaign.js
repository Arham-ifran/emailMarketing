...
import { Container, Form, Button, Modal, Table, Row, Col } from 'react-bootstrap';
...
function ViewCampaign(props) {
	...
	const getCampaign2 = (id) => {
		...
		axios.get(`/api/campaign/` + id + `/edit?lang=` + localStorage.lang, {
			...
			.then((res) => {
				...
				console.log(templateOptions.find(o => o.value == res.data.data.template_id));
				let img_link = templateOptions.find(o => o.value == res.data.data.template_id).img;
				console.log(img_link);
				setTemplateImg(img_link);
				console.log(img_link);
				var selectedOpt = options.find(o => o.value == res.data.data.group_id)
				var tempcontacts = [];
	...
	useEffect(() => {
		...
		if (parseUriSegment.indexOf('email-campaign') && parseUriSegment.indexOf('view') != -1) {
		...
		<React.Fragment>
			...
			<Container fluid>
				...
				<Form className="create-form-holder rounded-box-shadow bg-white">
					...
					<Form.Group className="mb-2 mb-md-4 d-flex flex-column flex-md-row">
						<Form.Label className="mb-2 mb-md-0" htmlFor="sender-email">{t('Sender Email Address')} <b className="req-sign">*</b></Form.Label>
						<div className="flex-fill input-holder">
							<input id="sender-email" className="form-control" type="text" disabled value={sender_email} placeholder="e.g. example@email.com" />
						</div>
					</Form.Group>

					<Form.Group className="mb-2 mb-md-4 d-flex flex-column flex-md-row">
						<Form.Label className="mb-2 mb-md-0" htmlFor="reply-to-email">{t('Reply to Address')} </Form.Label>
						<div className="flex-fill input-holder">
					...
					<Form.Group className="mb-2 mb-md-4 d-flex flex-column flex-md-row">
						<Form.Label className="mb-2 mb-md-0">{t('Select Template')}</Form.Label>
						<div className="flex-fill input-holder">
					...
					{
						campaignId && !hideStatus ?
							<Form.Group className="mb-2 mb-md-4 d-flex flex-column flex-md-row">
								<Form.Label className="mb-2 mb-md-0">{t('Status')} <b className="req-sign">*</b></Form.Label>
								<div className="flex-fill input-holder">
									<div className="subscriber-select w-100">
										<Select
											disabled
					...
					<Modal.Body className="d-flex align-items-center justify-content-center flex-column em-table">
						<div className="mb-3 group-select-title w-100 text-center">
							...
							<div className="selected-groups mt-3">
								{selectedGroups.map(group =>
									<span key={group.hash_id} className="chip"> {group.label} </span>
								)}
							</div>
						</div>
