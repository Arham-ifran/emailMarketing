...
import './assets/SplitTesting.css';
...
function SplitTesting(props) {
	...
	const campaignListing = () => {
		setLoading(true);
		axios.get('/api/split-testing/?page=' + pageNumber + '&filt_status=' + selectedOption + '&filt_camp_name=' + campaignName + '&filter_date=' + (filterCreated != '' ? moment.tz(filterCreated + " 12:00", localStorage.timezone).utc().format('YYYY-MM-DD') : '') + '&lang=' + localStorage.lang)
			.then(response => {
				if (response.data.status) {
					...
					axios.post('/api/split-testing/stats-show?lang=' + localStorage.lang)
						.then(res => {
							if (res.data.status) {
	...
	const deleteCampaign = () => {
		...
		setLoading(true);
		axios
			.delete("/api/split-testing/" + campaignId + "?lang=" + localStorage.lang)
			.then((response) => {
				setLoading(false);
				campaignListing();
				Swal.fire({
	...
	const handleStop = () => {
		...
		if (id) {
			setLoading(true);
			axios
				.post("/api/split-campaign/stop/" + id + "?lang=" + localStorage.lang)
				.then((response) => {
					setLoading(false);
					campaignListing();
					handleClose2();
	...
	const reportCampaignList = report.map((campaign, index) => {
		...
		return (
			<tr key={index}>
				...
				<td>{campaign.name ? campaign.name : 'No Name'}</td>
				<td>{campaign.split_test_param ? (campaign.split_test_param == 1 ? t("Subject") : t("Mail Content")) : t("Not Selected")}</td>
				...
				<td>
					<ul className="action-icons list-unstyled">
						<li>
							<Link to={`/split-testing/view/` + campaign.hash_id + "?page=" + pageNumber} className="view-icon" title={t("View")}><FontAwesomeIcon icon={faEye} /></Link>
						</li>
						{canViewReport && (campaign.status == 'Sending' || campaign.status == 'Sent' || campaign.status == "Stopped") ?
							<li>
								<Link to={`/split-testing/` + campaign.hash_id + `/report`} className="view-icon" title={t("report")}><FontAwesomeIcon icon={faListAlt} /></Link>
							</li>
							: ""}
						{campaign.status == "Draft" || campaign.status == "Active" ?
							<React.Fragment>
								<li>
									<Link to={`/split-testing/` + campaign.hash_id + `/edit` + "?page=" + pageNumber} className="edit-icon" title={t("Edit")}><FontAwesomeIcon icon={faPencilAlt} /></Link>
								</li>
							</React.Fragment>
							: ""}
				...
				<Container fluid>
					<div className="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between mb-3">
						<div className="page-title">
							<h1>{t('Split Testing')} </h1>
						</div>
						<div className="create-campaign">
							<Link to="/split-testing/create" className="btn btn-secondary">
						...
						<Form className="em-form" method="GET">
							...
							<Row>
								...
								<Col md="12" xs="12" className="d-flex justify-content-md-end">
									<Form.Group className="btn-wrapper filter-btns mb-2 mb-md-4 d-flex flex-column">
...
export default withTranslation()(SplitTesting);