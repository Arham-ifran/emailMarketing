...
<div class="main-content">
	<div class="content-heading clearfix">
		...
		<ul class="breadcrumb">
			...
			<li><a href="{{url('admin/split-campaigns')}}"><i class="fa fa-envelope"></i>Split Campaigns</a></li>
				...
				<table class="table table-hover" style="width:100%">
					...
					<tbody>
						...
						<tr>
							...
							<td> {{$contact['email']}} </td>
							<td> <span class="label label-{{$contact['pivot']['bounced_at']?'info':($contact['pivot']['sent_at']?'success':'danger')}}"> {{$contact['pivot']['bounced_at']?'Bounced':($contact['pivot']['sent_at'] ? "Sent" : "Failed")}} </span> </td>
						</tr>
						@endforeach
					</tbody>
					@endif
